...
All files used by HORNET, the extensions, Traefik & co will be stored in a directory called `data`.
...
You can start the HORNET node and extensions by running:
...
You can stop the HORNET node and extensions by running:
```sh
docker compose down
```
